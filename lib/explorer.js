...
module.exports = {
  ...
  get_hashrate: function(cb) {
    ...
    if (settings.nethash == 'netmhashps') {
      ...
      request({uri: uri, json: true, headers: {'User-Agent': 'nulliquidus'}}, function (error, response, body) { //returned in mhash
        if (body.netmhashps) {
      ...
      request({uri: uri, json: true, headers: {'User-Agent': 'nulliquidus'}}, function (error, response, body) {
