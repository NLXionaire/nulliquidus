...
function get_summary(coin, exchange, cb) {
    var summary = {};
    sleep9s;
    request({ uri: base_url + 's.html?a=volume&h=24&pair=' + coin + '-' + exchange, json: true, headers: {'User-Agent': 'nulliquidus'} }, function (error, response, body) {
        if (error) {
            ...
            sleep9s;
            request({ uri: base_url + '/' + coin + '-' + exchange + '.json', json: true, headers: {'User-Agent': 'nulliquidus'} }, function (error, response, body) {
                if (error) {
                    return cb(error, null);
...
function get_trades(coin, exchange, cb) {
    ...
    sleep9s;
    request({ uri: req_url, json: true, headers: {'User-Agent': 'nulliquidus'} }, function (error, response, body) {
