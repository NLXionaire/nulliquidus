...
function get_summary(coin, exchange, Crymktid, cb) {
  var summary = {};
  request({uri: base_url + '/' + Crymktid + '/ticker', json: true, headers: {'User-Agent': 'nulliquidus'}}, function (error, response, body) {
    if (error) {
      return cb(error, null);
    } else if (body.success === true) {
      summary['bid'] = body.data['bid'].toFixed(8);
      summary['ask'] = body.data['ask'].toFixed(8);
      request({uri: base_url + '/' + Crymktid, json: true, headers: {'User-Agent': 'nulliquidus'}}, function (error, response, body) {
        if (error) {
          return cb(error, null);
        } else if (body.success === true) {
